...
import './catalog-page-template.css'
...
function CatalogCard({ item }: { item: CatalogCardItem }) {
  const content = (
    <>
      <img src={item.imageUrl || item.fallbackImageUrl} alt={item.name} className="catalog-card__logo" />
      <h3 className="catalog-card__name">{item.name}</h3>
      {item.subtitle && <p className="catalog-card__subtitle">{item.subtitle}</p>}
    </>
  )

  if (item.linkTo) {
    return (
      <Link to={item.linkTo} className="catalog-card__link">
        {content}
      </Link>
    )
  }

  return <div className="catalog-card__body">{content}</div>
}
  ...
  return (
    <section>
      ...
      <ul className={`catalog-grid catalog-grid--${variant}`}>
        ...
        {items.length === 0 && <li className="catalog-grid__empty">{emptyText}</li>}
      </ul>
